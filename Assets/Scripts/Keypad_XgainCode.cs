...
public class Keypad_XgainCode : MonoBehaviour {
...
//        Debug.Log("gksdjf sdlkfj   " + dugh);
        ...
        //******xxx.xx
        {
            ...
            if (temp[0] == "1")
            {
                ...
                if(temp[1] == "1")
                {
                    ...
                    //*******If it makes it here the first, and second integer must be a 1. The 3rd integer better be either a 1,2,3, or 4.
                    //*******11_.x
                    {
                        formatError(temp.Count);
                        return false;
    ...
    public void plus6dB_x(double x)
    {
        myElemInDoubleForm = x;
        
        xText.GetComponent<Text>().text = myElemInDoubleForm.ToString("0.0");
        //update gain button in main menue
        Options_Menu.GainXText(myElemInDoubleForm.ToString("0.0"));
    ...
    void saveList()
    {
        ...
        //update gain button in main menue
        xText.text = myUpdatedTextElement;
     
        
        if (colorChangeNumber == 1)
        {
        ...
        {
            //if button is not red only update X
            Panel_Gain.UpdateXValue(myElemInDoubleForm);
            Options_Menu.GainXText(myUpdatedTextElement);
